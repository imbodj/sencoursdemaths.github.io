...
% Premier graphique : y = √x pour x ≥ 1
...
% Deuxième graphique : y = -√x pour x ≥ 1
\begin{tikzpicture}[scale=0.4]
    % Définition des limites
    \def\xmin{-1}
    \def\xmax{8}
...
% Troisième graphique : y = √(-x) pour x ≤ -1
\begin{tikzpicture}[scale=0.4]
    % Définition des limites
    \def\xmin{-8}
    \def\xmax{1}
    ...
    % Fonction y = √(-x) pour x ≤ -1
    \draw[red, thick, domain=\xmin:-1, samples=200] plot (\x, {sqrt(-\x)});
\end{tikzpicture}

\vspace{1cm}

% Quatrième graphique : y = -√(-x) pour x ≤ -1.2
\begin{tikzpicture}[scale=0.4]
    % Définition des limites
    \def\xmin{-8}
    \def\xmax{1}
    ...
    % Fonction y = -√(-x) pour x ≤ -1.2
    \draw[red, thick, domain=\xmin:-1.2, samples=200] plot (\x, {-sqrt(-\x)});
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.4]
    ...
    % Fonction y = x² pour x ≥ 1
    \draw[red, thick, domain=1:\xmax, samples=200] plot (\x, {\x*\x});
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.4]
    ...
    % Fonction y = -x² pour x ≤ -1.2
    \draw[red, thick, domain=\xmin:-1.2, samples=200] plot (\x, {-\x*\x});
\end{tikzpicture}
...
\text{Si } f(x) \text{ admet un } \textbf{\color{magenta}\text{minimum positif }} \\
\text{sur } I \text{ alors } f \text{ est } \textbf{\color{magenta} \text{positive}} \text{ sur } I.
...
\text{Si } f(x) \text{ admet un } \textbf{\color{magenta}\text{maximum négatif }} \\
\text{sur } I \text{ alors } f \text{ est } \textbf{\color{magenta} \text{négative}} \text{ sur } I.
...
f(x) \text{ est } \textbf{\color{magenta} \text{positif}} \text{ si } \textbf{\color{magenta}x \geq \alpha}. \\
f(x) \text{ est } \textbf{\color{magenta}\text{négatif}} \text{ si } \textbf{\color{magenta}x \leq \alpha}.
...
f(x) \text{ est } \textbf{\color{magenta} \text{négatif}} \text{ si } \textbf{\color{magenta}x \geq \alpha}. \\
f(x) \text{ est } \textbf{\color{magenta}\text{positif}} \text{ si } \textbf{\color{magenta} x\leq\alpha}.
```}
...
\subsubsection*{Position d'une courbe par rapport à sa tangente}
  ...
  \begin{tikzpicture}[scale=0.5]
    ...
    % Tangente y = -3x + 1
    \draw[blue, thick, domain=-2.5:2.5] 
        plot (\x, {-3*\x + 1});
